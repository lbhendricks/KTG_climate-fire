...
print(paste(sum(is.na(KTG$wind_speed_knots)),"NAs in wind speed"))
```
So, it's not perfect but it IS better than the original data! Might need to do something like break this into sections that start and stop before and after the big chunks of missing data.... Need to talk with Dan about that. But that shouldn't affect us until we get to the aet coalculations, since that does rely on the values from previuos days. 
...
ET0_output<-dailyET0(lat=lat,elev=elev,psun=KTG$psun,wind=KTG$wind_speed_ms,doy=KTG$doy,tmax=KTG$T_max,tmin=KTG$T_min,rh=KTG$humidity)

# using Dobrowski et al version
ET0_output_Dobrowski<-dailyET0_Dobrowski(radiation=KTG$radiation,tmax=KTG$T_max,tmin=KTG$T_min,wind=KTG$wind_speed_ms,lat=lat,elev=elev,dpt=KTG$dewpoint,doy=KTG$doy)
...
soilbalance122<-aetmod(et0=ET0_output$et0,precip=KTG$precip,awc=122)
 
# then plot the aet
plot(soilbalance$aet,type="l")
plot(soilbalance$aet[1:500],type="l")


soilbalance50<-aetmod(et0=ET0_output$et0,precip=KTG$precip,awc=50)
...
```{r}
KTG$ET0<-ET0_output$et0
KTG$aet<-soilbalance$aet
KTG$deficit<-soilbalance$def
KTG$soil<-soilbalance$soil
...
write.table(KTG,"/Users/laurenhendricks/Documents/Borneo/Ketapang_ClimateFire/Ketapang_ET0andAET.txt",sep=",",row.names = F)
```
...
print(paste(sum(is.na(KTG_2000$T_min)),"NAs in T_min"))
print(paste(sum(is.na(KTG_2000$T_max)),"NAs in T_max"))
print(paste(sum(is.na(KTG_2000$T_avg)),"NAs in T_avg"))
print(paste(sum(is.na(KTG_2000$humidity)),"NAs in humidity"))
print(paste(sum(is.na(KTG_2000$precip)),"NAs in precip"))
print(paste(sum(is.na(KTG_2000$sunshine)),"NAs in sunshine hours"))
print(paste(sum(is.na(KTG_2000$wind_speed_ms)),"NAs in wind speed"))
print(paste(sum(is.na(KTG_2000$ET0)),"NAs in ET0"))
```
So, all of the 
...
plot(KTG_2000$date,KTG_2000$ET0,type="p",pch=16,cex=0.25,ylab="ET0 ",xlab="Date")

# just for fun look at ET0 vs aet
plot(KTG_2000$ET0,KTG_2000$aet)

# also plot soil deficit instead of aet
plot(KTG_2000$date,KTG_2000$deficit,type="p",pch=16,cex=0.25,ylab="Deficit",xlab="Date")
...
Neot DEFICIT, not aet, against FRP. Deficit is most similar to what is used in a fire index. 
